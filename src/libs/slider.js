...
var SliderFactory = function(container,options){
...
$.extend(SliderFactory.prototype,{
    ...
    attachTo : function(obj, options) {
        ...
        return obj.each(function(n,item){
            if (!item.getAttribute('l')) {
                ...
                SliderFactory.cache.push(new SliderFactory(item, options));
            }
        });
    }
});
SliderFactory.cache = [];
SliderFactory.destroy = function(){
    var cache = SliderFactory.cache,
        len = cache.length;
    if(len < 1){return;}
    for(var i=0;i<len;i++){
        cache[i].destroy();
