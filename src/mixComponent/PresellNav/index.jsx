...
export default class PresellNav extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    componentDidMount() {
        const parentOffsetLeft = this.scrollBar.offsetLeft;
        const offsetLeft = this.scrollBar.querySelector('.active').offsetLeft;
        this.scrollBar.scrollLeft = offsetLeft - parentOffsetLeft;
    }
    ...
    render() {
        ...
        if ( data && data.length > 0 ) {
            return (
                <div className="presell-nav">
                    <ul ref={(el) => { this.scrollBar = el; }}>
                        {
                            data.map((time, i) => {
                                const showTimeText = moment(time).format('MM月DD日');
